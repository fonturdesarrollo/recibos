<?php
	...
	$consulta = pg_query($conexion, "SELECT * FROM \"SIMA016\".nphiscon WHERE cedemp = '$cedula' AND cuenta_banco = '$numero_cuenta' AND fecnom BETWEEN '$fecha_inicial' AND '$fecha_final' AND especial = '$tipo_nomina' LIMIT 1;");
	if (!$consulta) {
	...
	while ($fila = pg_fetch_array($consulta)) {
		...
		if($codigo_nomina=="010"&&$codigo_nomina=="004"){
			echo '<META HTTP-EQUIV="Refresh" Content="0; URL=ingreso.php?error=rne">'; exit;
		}
		...
		$consulta1 = pg_query($conexion, "SELECT fecing FROM \"SIMA016\".nphojint WHERE cedemp = '$cedula_empleado';");
		//echo "SELECT fecing FROM \"SIMA016\".nphojint WHERE cedemp = '$cedula_empleado';";
		if (!$consulta1) {
			//echo "No se pudo ejecutar la consulta."; exit;
		}
		...
		$consulta2 = pg_query($conexion, "SELECT nomnom FROM \"SIMA016\".npnomina WHERE codnom = '$codigo_nomina';");
		//echo "SELECT nomnom FROM \"SIMA016\".npnomina WHERE codnom = '$codigo_nomina';";
		if (!$consulta2) {
			//echo "No se pudo ejecutar la consulta."; exit;
		}
		...
		$consulta3 = pg_query($conexion, "SELECT monto FROM \"SIMA016\".nphiscon WHERE cedemp = '$cedula' AND cuenta_banco = '$numero_cuenta' AND nomcon = 'SUELDO BASICO QUINCENAL' AND fecnom BETWEEN '$fecha_i' AND '$fecha_f' AND especial = 'N' AND now()>=fecnom;");
		//echo "SELECT monto FROM \"SIMA016\".nphiscon WHERE cedemp = '$cedula' AND cuenta_banco = '$numero_cuenta' AND nomcon = 'SUELDO BASICO QUINCENAL' AND fecnom BETWEEN '$fecha_i' AND '$fecha_f' AND especial = 'N' AND now()>=fecnom;";
		if (!$consulta3) {
			//echo "No se pudo ejecutar la consulta."; exit;
		}
	...
	$consulta_asignaciones = pg_query($conexion, "SELECT nomcon, monto FROM \"SIMA016\".nphiscon WHERE cedemp = '$cedula' AND cuenta_banco = '$numero_cuenta' AND ASIDED = 'A' AND fecnom BETWEEN '$fecha_inicial' AND '$fecha_final' AND especial = '$tipo_nomina' ORDER BY codcon ASC;");
	...
	$consulta_deducciones = pg_query($conexion, "SELECT nomcon, monto FROM \"SIMA016\".nphiscon WHERE cedemp = '$cedula' AND cuenta_banco = '$numero_cuenta' AND ASIDED = 'D' AND fecnom BETWEEN '$fecha_inicial' AND '$fecha_final' AND especial = '$tipo_nomina' ORDER BY codcon ASC;");
...
$pdf->SetFont('Times','',9);
$pdf->Cell(66,6,$cuenta_banco,0,0,'L');
$pdf->SetFont('Times','B',9);
if($tipo_nomina=="N"){
